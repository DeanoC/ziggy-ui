//! Runtime state for the theme engine
...
pub const WorkspaceLayoutPreset = struct {
    // Panel layout for workspace
    panels: [10][]const u8 = undefined,
    panels_len: u8 = 0,
    focused: ?[]const u8 = null,
    ...
    pub fn openPanels(self: *const WorkspaceLayoutPreset) [][]const u8 {
...
pub fn allowHover() bool {
    const p = getProfile();
    return p.allow_hover_states;
}
...
pub fn setStyleSheets(light_sheet: StyleSheet, dark_sheet: StyleSheet) void {
    active_styles_light = light_sheet;
    active_styles_dark = dark_sheet;
}
