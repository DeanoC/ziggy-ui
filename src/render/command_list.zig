...
/// Command list for recording draw operations
pub const CommandList = struct {
    ...
    pub fn getTextSlice(self: *const CommandList, cmd: TextCmd) []const u8 {
        const start = cmd.text_offset;
        const end = start + cmd.text_len;
        if (end > self.text_storage.items.len) return "";
        return self.text_storage.items[start..end];
    }

    pub fn iter(self: *const CommandList) Iterator {
        return .{ .list = self, .index = 0 };
    }

    pub const Iterator = struct {
        list: *const CommandList,
        index: usize,

        pub fn next(self: *Iterator) ?Command {
            if (self.index >= self.list.commands.items.len) return null;
            const cmd = self.list.commands.items[self.index];
            self.index += 1;
            return cmd;
        }
...
/// Command queue for multi-frame command buffering
pub const CommandQueue = struct {
    allocator: std.mem.Allocator,
    lists: std.ArrayList(CommandList),
    ...
    pub fn beginFrame(self: *CommandQueue) !*CommandList {
        if (self.active_index >= self.lists.items.len) {
            try self.lists.append(self.allocator, CommandList.init(self.allocator));
        }
        const list = &self.lists.items[self.active_index];
        list.clear();
        return list;
    }

    pub fn endFrame(self: *CommandQueue) void {
        self.active_index += 1;
    }
    ...
    pub fn getActive(self: *CommandQueue) ?*CommandList {
        if (self.active_index >= self.lists.items.len) return null;
        return &self.lists.items[self.active_index];
    }
};
