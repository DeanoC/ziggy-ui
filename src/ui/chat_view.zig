...
fn fieldType(comptime T: type, comptime name: []const u8) type {
    ...
    if (info != .@"struct") {
        @compileError("ChatView Message must be a struct type");
    }
    inline for (info.@"struct".fields) |field| {
        if (std.mem.eql(u8, field.name, name)) return field.type;
    }
    @compileError(std.fmt.comptimePrint("ChatView Message is missing required field '{s}'", .{name}));
}
...
fn expectByteSliceField(comptime Container: type, comptime field_name: []const u8) void {
    const ty = fieldType(Container, field_name);
    if (!isByteSlice(ty)) {
        @compileError(std.fmt.comptimePrint(
            "ChatView field '{s}' must be a byte slice ([]const u8 or []u8)",
            .{field_name},
        ));
    }
}

fn expectOptionalByteSliceField(comptime Container: type, comptime field_name: []const u8) void {
    const ty = fieldType(Container, field_name);
    const child = switch (@typeInfo(ty)) {
        .optional => |o| o.child,
        else => @compileError(std.fmt.comptimePrint("ChatView field '{s}' must be optional", .{field_name})),
    };
    if (!isByteSlice(child)) {
        @compileError(std.fmt.comptimePrint(
            "ChatView field '{s}' must be ?[]const u8 (or ?[]u8)",
            .{field_name},
        ));
    }
}

fn validateLocalStateType(comptime LocalState: type) void {
    const info = @typeInfo(LocalState);
    if (info != .@"enum") {
        @compileError("ChatView Message.local_state must be an enum with 'sending' and 'failed' tags");
    }

    var has_sending = false;
    var has_failed = false;
    inline for (info.@"enum".fields) |field| {
        if (std.mem.eql(u8, field.name, "sending")) has_sending = true;
        if (std.mem.eql(u8, field.name, "failed")) has_failed = true;
    }

    if (!has_sending or !has_failed) {
        @compileError("ChatView Message.local_state enum must include 'sending' and 'failed' tags");
    }
}

fn attachmentType(comptime Message: type) type {
    const ty = fieldType(Message, "attachments");
    const slice_ty = switch (@typeInfo(ty)) {
        .optional => |o| o.child,
        else => @compileError("ChatView Message.attachments must be optional (?[]Attachment)"),
    };

    const pointer = switch (@typeInfo(slice_ty)) {
        .pointer => |p| p,
        else => @compileError("ChatView Message.attachments must be a slice (?[]Attachment)"),
    };

    if (pointer.size != .slice) {
        @compileError("ChatView Message.attachments must be a slice (?[]Attachment)");
    }
    return pointer.child;
}

fn validateAttachmentType(comptime Attachment: type) void {
    expectByteSliceField(Attachment, "kind");
    expectByteSliceField(Attachment, "url");
    expectOptionalByteSliceField(Attachment, "name");
}

fn validateMessageType(comptime Message: type) void {
    expectByteSliceField(Message, "id");
    expectByteSliceField(Message, "role");
    expectByteSliceField(Message, "content");

    if (fieldType(Message, "timestamp") != i64) {
        @compileError("ChatView Message.timestamp must be i64");
    }

    const Attachment = attachmentType(Message);
    validateAttachmentType(Attachment);

    const local_state_ty = fieldType(Message, "local_state");
    const local_state_child = switch (@typeInfo(local_state_ty)) {
        .optional => |o| o.child,
        else => @compileError("ChatView Message.local_state must be optional (?LocalChatMessageState)"),
    };
    validateLocalStateType(local_state_child);
}

pub fn ChatView(comptime Message: type) type {
    comptime validateMessageType(Message);
    const Attachment = attachmentType(Message);
    ...
    return struct {
        ...
        pub fn deinit(state: *@This().ViewState, allocator: std.mem.Allocator) void {
        ...
        pub fn hasSelectCopySelection(state: *const @This().ViewState) bool {
            if (state.select_copy_editor) |editor| {
        ...
        pub fn copySelectCopySelectionToClipboard(allocator: std.mem.Allocator, state: *@This().ViewState) void {
            if (state.select_copy_editor) |*editor| {
                _ = editor.copySelectionToClipboard(allocator);
            }
        }

        pub fn hasSelection(state: *const @This().ViewState) bool {
        ...
        pub fn drawCustom(
            ...
            if (copy_requested and state.focused) {
                @This().copySelectionToClipboard(allocator, state, messages, stream_text, inbox, opts.show_tool_output);
            }
        ...
        fn isImageAttachment(att: Attachment) bool {
        ...
        fn attachmentStateHash(attachments: ?[]const Attachment) u64 {
        ...
        fn clearMessageCache(state: *@This().ViewState, allocator: std.mem.Allocator) void {
            if (state.message_cache == null) return;
        ...
        fn ensureMessageCacheMap(state: *@This().ViewState, allocator: std.mem.Allocator) *std.StringHashMap(MessageCache) {
        ...
        fn isEmptyMessage(msg: Message) bool {
            if (msg.attachments) |attachments| {
        ...
        fn selectionRange(state: *const @This().ViewState) ?[2]usize {
...
test "ChatView accepts compatible message type" {
    const Attachment = struct {
        kind: []const u8,
        url: []const u8,
        name: ?[]const u8 = null,
    };
    const MsgState = enum { sending, failed };
    const Msg = struct {
        id: []const u8,
        role: []const u8,
        content: []const u8,
        ...
        local_state: ?MsgState = null,
    };

    const GenericChatView = ChatView(Msg);
    var view_state: GenericChatView.ViewState = .{};
    _ = &view_state;
}

pub const DefaultChatView = ChatView(zsc.protocol.types.ChatMessage);

pub const ViewState = DefaultChatView.ViewState;
pub const deinit = DefaultChatView.deinit;
pub const hasSelectCopySelection = DefaultChatView.hasSelectCopySelection;
pub const copySelectCopySelectionToClipboard = DefaultChatView.copySelectCopySelectionToClipboard;
pub const hasSelection = DefaultChatView.hasSelection;
pub const copyAllToClipboard = DefaultChatView.copyAllToClipboard;
pub const copySelectionToClipboard = DefaultChatView.copySelectionToClipboard;
pub const drawSelectCopy = DefaultChatView.drawSelectCopy;
pub const drawCustom = DefaultChatView.drawCustom;
pub const deinitGlobals = DefaultChatView.deinitGlobals;
